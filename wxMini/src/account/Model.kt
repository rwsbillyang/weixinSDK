 ...
 *
 * Written by rwsbillyang@qq.com at Beijing Time: 2022-11-30 16:42
 *
...
@Serializable
@Resource("/list")
data class WxMiniAccountListParams(
    ...
    override fun toFilter(): Bson {
        val idFilter = _id?.let { WxMiniAccount::_id eq it.toObjectId() }
        val statusF = if (status != "-1") WxMiniAccount::state eq status else null
        val openIdF = openId?.let { WxMiniAccount::openId eq it }

        val appIdF = appId?.let { WxMiniAccount::appId eq it }
